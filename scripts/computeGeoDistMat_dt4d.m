...
sourcepath = '../exp/data/';
dataset = 'DT4D_r'; %name of the dataset
filetype = 'obj';

categories = ["crypto", "mannequin", "ninja", "prisoner", "skeletonzombie", "drake", "mousey", "ortiz", "pumpkinhulk", "zlorp"];

for cid=1:size(categories, 2)
    datafolder = [sourcepath dataset '/shapes/' convertStringsToChars(categories(cid)) '/'];
    ...
    for pts = 1:n_files
        ...
        S1 = MESH.MESH_IO.read_shape(Vname);
        S1 = MESH.compute_LaplacianBasis(S1, max_spec_dim);
        sqrt_area = S1.sqrt_area;
        geodist = MESH.compute_geodesic_dist_matrix(S1);

        savefolder_geodist = [sourcepath dataset '/geodist/' convertStringsToChars(categories(cid)) '/'];
